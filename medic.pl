...
sub cmd_medic {
	...
	Irssi::print('%G>>%n cs_watch         : This is the channel we watch for medic events.');
	Irssi::print('%G>>%n cs_medic_keyword : This is the regex we use to look for medic events.');
	Irssi::print('%G>>%n cs_alert         : This is who we alert for medic events.');
}

# The channel we're watching
Irssi::settings_add_str($IRSSI{'name'}, 'cs_watch', '');
# Regex for the medic command
Irssi::settings_add_str($IRSSI{'name'}, 'cs_medic_keyword', '^!medic');
# Who we're alerting
...
sub sig_public {
	...
	my $keyword = Irssi::settings_get_str('cs_medic_keyword');
